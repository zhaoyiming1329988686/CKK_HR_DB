...
namespace UI.Controllers.人力资源档案管理.人力资源档案删除管理
{
    ...
    /// 2.档案删除恢复
    /// </summary>
    public class human_recovery_locateController : Controller
    {
        //一级机构表
        ...
        public ActionResult IndexSelect2()
        {
            ...
            List<human_file> list = hhff.human_fileFenYe(e => e.huf_id, e => e.check_status == 1 && e.human_file_status == false, out rows, out pages, currentPage, pageSize);
        ...
        //3.按编号单个详细显示  恢复页面
        public ActionResult HuiFuIndex()
        {
        ...
        public ActionResult HuiFu()
        {
            ...
            hf.recovery_time = DateTime.Now;//档案恢复时间
            hf.human_file_status = true;//档案状态
            if (hhff.human_fileUpdate(hf) > 0)
            {
                return Content("<script>alert('人力资源档案恢复成功！');location.href='/human_recovery_locate/Index';</script>");
            }
            else
            {
                return View(hf);
